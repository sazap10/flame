...
Backend tests run via the built-in Node test runner (`npm test` → `node --test`), with files next to the code as `*.test.js` (e.g. `controllers/apps/docker/parseLabels.test.js`). Client tests run via **Vitest + React Testing Library** in jsdom (`npm test` / `npm run test:watch` from `client/`), with files as `*.test.tsx` next to the component and a `renderWithStore` helper in `client/src/testing/test-utils.tsx` that wraps the component in a real Provider store. The focus is regression guards for `useEffect` behavior (e.g. a socket/timer must not be re-created on re-render); note that an effect→setState *infinite loop* can't be guarded in-process because it blocks the event loop, so those aren't tested. Biome owns both linting and formatting (Prettier has been removed): `npm run lint`, `npm run format` (writes), `npm run check` (lint+format+import-sort, writes), or `npm run ci` for the non-mutating check CI uses. `biome.json` runs the full `recommended` preset (formatting matches the old Prettier style: 2-space, single quotes, `trailingCommas: es5`, `lineWidth` 80, always-arrow-parens); two rules are relaxed — `a11y/useSemanticElements` (off, the codebase uses styled `role="button"` controls with keyboard handlers) and `correctness/useExhaustiveDependencies` (off, its autofix rewrites dependency arrays in unsafe ways). The client uses Vite (not Create React App); the build is bundled by esbuild/Rolldown and does not run `tsc`, so run `npm run typecheck` separately to catch type errors. CI (`.github/workflows/ci.yml`) runs backend tests, Biome, and the client type-check + Vitest tests on every push/PR.
...
- **Docker** — `controllers/apps/docker/useDocker.js` reads container labels (`flame.type`, `flame.name`, `flame.url`, `flame.icon`, `flame.category`; `;`-separated for multiple) and auto-creates **apps grouped into categories** (not bookmarks). `flame.category` sets the category; containers without it fall back to `DOCKER_DEFAULT_CATEGORY` (default `Apps`). Newly discovered apps are pinned so they show on the home page; categories are created on demand, and any bookmark whose `name` and `url` both match a discovered app is deleted so the service isn't shown twice (the match is on name+url, not on tracked provenance). Discovery runs from `controllers/categories/getAllCategories.js` (gated by the "Use Docker API" setting) rather than during app fetching: categories must exist before their contents are read, and that public endpoint is hit on every home page load, making it a reliable trigger that also surfaces freshly created categories in the same request. It is throttled to once per 30s. Requires the Docker socket mounted. URLs can also be derived from `traefik.*` rules, or from `tsdproxy.*` labels when `TSDPROXY_DOMAIN` is set (`tsdproxy.name` → `https://<name>.<TSDPROXY_DOMAIN>`); explicit `flame.*` labels always override the derived values.
