### Unreleased
- Apps and bookmarks can now use icon shorthands in any icon field (and through Docker/Kubernetes `flame.icon` labels): `selfhst:<name>` for [selfh.st/icons](https://selfh.st/icons/) (e.g. `selfhst:bitwarden`), and `gh:<owner>/<repo>/<name>` for any GitHub repo that follows the selfh.st folder layout (e.g. `gh:sazap10/personal-project-icons/flame`). Both accept an optional `/png`|`/webp` format and/or `/auto` to pick the `-light`/`-dark` variant based on the active theme. SVG icons are recoloured to match the active theme; PNG/webp keep their original colours
...
- Docker discovery now reads optional `flame.icon.light` and `flame.icon.dark` labels to set per-scheme icons on auto-created bookmarks (`;`-separated for multi-app labels, falling back to `flame.icon`) ([#7](https://github.com/sazap10/flame/pull/7))
