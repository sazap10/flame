### Unreleased
- Apps and bookmarks can now use [selfh.st/icons](https://selfh.st/icons/) via a `selfhst:<name>` shorthand in any icon field (e.g. `selfhst:bitwarden`, or `selfhst:bitwarden/png` to pick the format); also works through Docker/Kubernetes `flame.icon` labels. SVG icons are recoloured to match the active theme; PNG/webp keep their original colours. Append `/auto` (e.g. `selfhst:bitwarden/auto`) to automatically pick selfh.st's `-light`/`-dark` monochrome variant based on the active scheme
